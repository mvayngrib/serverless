      ...
      functionObj.events.forEach(event => {
        ...
        if (typeof cloudwatchLog === 'object') {
          ...
          if (typeof logGroup === 'string') {
            // e.g. "/aws/lambda/mystackname-hello"
            LogGroupName = stripLineBreaks(logGroup);
          } else {
            // e.g. { "Fn::Sub": "/aws/lambda/${AWS::StackName}-hello" }
            LogGroupName = JSON.parse(stripLineBreaks(JSON.stringify(logGroup)));
          }
        ...
        if (typeof LogGroupName === 'string') {
          ...
          LogGroupName = JSON.stringify(LogGroupName);
        }
