...
module.exports = {
  ...
  getLogGroupTemplateName(functionName) {
    const shortName = this.getLambdaShortName(functionName);
    const stackNameRef = '${AWS::StackName}';
    return `/aws/lambda/${stackNameRef}-${shortName}`;
  },
  ...
  getLambdaShortName(functionName) {
    const prefixLength = this.getStackName().length + 1;
    return functionName.slice(prefixLength);
  },
