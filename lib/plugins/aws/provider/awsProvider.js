...
const LOCAL_ENDPOINTS = require('../lib/localstack')
...
class AwsProvider {
  ...
  request(service, method, params) {
    ...
    if (IS_LOCAL) {
      console.log('targeting LOCALSTACK')
      if (service in LOCAL_ENDPOINTS) {
        credentials.endpoint = LOCAL_ENDPOINTS[service];
      } else {
        console.warn(`LOCALSTACK ${service} mapping not found`);
      }
    }
